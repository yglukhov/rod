...
import nimx / [ view, types, layout, event, view_event_handling, image, context ]
import nimx/pasteboard/pasteboard_item
import nimx/assets/asset_loading
import ../../[editor_view_types, editor_project_settings]
import ../../scene/components/[grid, viewport_rect, editor_component]
import ../../scene/[gizmo, gizmo_move, node_selector, editor_camera_controller]
import ../../../[node, rod_types, viewport]
import ../../../tools/debug_draw
import ../../../component/[ sprite, camera ]
...
method onInterceptTouchEv*(v: EditorSceneView, e: var Event): bool = discard#not v.editor.sceneInput
  ...
  of bsDown:
    ...
    if not gizmoTouch:
      ...
      if not castedNode.isNil:
        ...
        v.postNodeSelected(castedNode)
        # v.editor.selectedNode = castedNode
      else:
        v.postNodeSelected(nil)
...
method onCompositionChanged*(v: EditorSceneView, c: CompositionDocument) =
  procCall v.EditorTabView.onCompositionChanged(c)
  if c.isNil:
    return
  v.selectedNode = v.rootNode
  if not v.gizmo.isNil:
    v.gizmo.editedNode = v.rootNode
  v.nodeSelector.selectedNode = v.rootNode
  v.compPlaceholder.removeAllChildren()
  v.compPlaceholder.addChild(v.rootNode)
  # echo "EditorSceneView onCompositionChanged "

method onEditorEvent*(v: EditorSceneView, ev: EditorAPIEvent) = discard

method setInspectedNode*(v: EditorSceneView, n: Node) =
  v.selectedNode = n
...
method init*(v: EditorSceneView) =
  ...
  v.makeLayout:
    ...
    - SceneView as sceneview:
      frame == super
      rootNode: newNode(EditorRootNodeName)
  ...
  for camSettings in cameraSettings:
    let camNode = sceneview.rootNode.newChild(camSettings.name)
  ...
  var grid = v.sceneView.rootNode.component(EditorGrid)
  v.sceneView.editing = true
  discard sceneview.rootNode.newChild("overlay").component(ViewportRect)
  v.compPlaceholder = sceneview.rootNode.newChild("placeholder")
